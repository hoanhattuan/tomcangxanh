...
thresholdController.get("/danhsachnguong",service.ensureAuthenticated, function(req, res) {
  res.render("expert/nguong/danhsachnguong", {title: 'Xem danh sách ngưỡng',secu:config.securitycode,conf:config.urladdress,token:req.session.token,userid:req.session.userid,username:req.session.username,fullName:req.session.fullName});
});
thresholdController.get("/themnguong",service.ensureAuthenticated,function(req,res){
    res.render("expert/nguong/themnguong",{title: 'Thêm ngưỡng',secu:config.securitycode,conf:config.urladdress,token:req.session.token,userid:req.session.userid,username:req.session.username,fullName:req.session.fullName})
});
thresholdController.post("/themnguong",function(req,res){
    ...
    service.post(options,function(error,body){
      ...
      res.redirect('/quantrac/chuyengia/nguong/danhsachnguong');
    });
});
thresholdController.get("/capnhatnguong/:id",service.ensureAuthenticated,function(req,res){
    ...
    service.get(options,function(error,data){
      if(error){
        ...
        res.render("expert/nguong/capnhatnguong",{title: 'Cập nhật ngưỡng',secu:config.securitycode,conf:config.urladdress,token:req.session.token,userid:req.session.userid,username:req.session.username,thresholdData:thresholdData,threshold_type:threshold_type,fullName:req.session.fullName});
      }
    });
});
