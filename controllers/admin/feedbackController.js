...
feedbackController.post('/them',service.ensureAuthenticated,function(req,res){
	...
	service.post(options,function(error,data){
		if(error){
			...
			res.redirect('/quantrac/nguoiquantri/phanhoi/danhsach');
		} 
	});
});
...
feedbackController.get('/xemchitiet/:id',service.ensureAuthenticated,function(req,res){
	var token = "JWT " + req.session.token;
	var id = req.params.id; 
	var options = service.setOption('GET',config.urladdress + '/api/feedback/getbyid/' + id,{'Authorization':token});
	service.get(options,function(error,data){
		if(error){
			...
			userData = data.data;
			console.log(userData);
			res.render('admin/seeFeedback',{title:'Xem chi tiết phản hồi',users:userData,secu:config.securitycode,conf:config.urladdress,token:req.session.token,userid:req.session.userid,username:req.session.username,fullName:req.session.fullName});
		} 
	});
});
