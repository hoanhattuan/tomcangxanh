...
postController.post('/them',service.ensureAuthenticated,function(req,res){
	...
	service.post(options,function(error,data){
		if(error){
			...
			res.redirect('/nguoiquantri/baiviet/danhsach');
		} 
	});
});
...
postController.post('/sua/:id',service.ensureAuthenticated,function(req,res){
	console.log('Vao trong ruot');
	var token = "JWT " + req.session.token;
	var body ={
		user_id : 1,
