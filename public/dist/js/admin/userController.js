...
userController.get("/danhsach",service.ensureAuthenticated, function(req, res) {
	res.render("admin/ListUser2", {title: 'Xem danh sách người dùng',secu:config.securitycode,conf:config.urladdress,token:req.session.token,userid:req.session.userid,username:req.session.username,fullName:req.session.fullName});
});
...
userController.post('/sua/:id',service.ensureAuthenticated,function(req,res){
	...
	service.put(options,function(error,data){
		if(error){
			...
			res.redirect('/nguoiquantri/nguoidung/danhsach');
		} 
	});
});
